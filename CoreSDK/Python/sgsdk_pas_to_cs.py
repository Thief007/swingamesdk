...
def args_to_call_str(args):
    str = []
    for arg in args:
        # print arg
        modifier, name, type = arg
        str.append("%s %s" % (modifier, name)) 
        #str.append("%s" % type) 
    return ', '.join(str)
...
'''
    ...
    for sig in sig_tuples:
        ...
        f.write('\n')
        f.write('\t\tpublic static %s %s(%s)\n' % (retn, name, args_to_str(args)))
        f.write('\t\t{\n')
        if retn.find('void') < 0:
            f.write('\t\t\t%s localResult;\n' % (retn))
        f.write('\t\t\ttry\n')
        f.write('\t\t\t{\n')
        if retn.find('void') >= 0:
            f.write('\t\t\t\tDLL_%s(%s);' % (name, args_to_call_str(args)))
        else:
            f.write('\t\t\t\tlocalResult = DLL_%s(%s);' % (name, args_to_call_str(args)))
        body = '''
            }
            catch (Exception exc)
            {

...
FPC_BIN=`which fpc`
if [ "$OS" = "$WIN" ]; then
    PAS_FLAGS="-g -Ci -gc -Ct -dSWINGAME_SDL13"
else
    PAS_FLAGS="-gw -Ci -Ct -dSWINGAME_SDL13"
fi
...
doWindowsCompile()
{
    ...
    ${FPC_BIN}  ${PAS_FLAGS} ${SG_INC} -Mobjfpc -gh -Sh -FE${OUT_DIR} -FU${TMP_DIR} -Fu${LIB_DIR} -Fi${LIB_DIR} -o${GAME_NAME}.exe ./test/${SRC_FILE} > ${LOG_FILE}

...
{ function CalculateAngleSprite(sprite1, sprite2: Sprite): Single;  cdecl; export;
  ...
  begin
    Try
      ...
      //WriteLn('in Pascal = ', result.x, ':', result.y);
      
      {$IFDEF TRACE}
        Trace('SGSDK.dll', 'GetSpriteMovement', ' returns (x:y)', FloatToStr(result.x) + ':' + FloatToStr(result.y));
      {$ENDIF}
  ...
  begin
    Try
      {$IFDEF TRACE}
        Trace('SGSDK.dll', 'SetSpriteMass', ' ', FloatToStr(mass));
      {$ENDIF}
  ...
  begin
    Try
      {$IFDEF TRACE}
        Trace('SGSDK.dll', 'SetSpriteMovement', ' set to (x:y) ', FloatToStr(v.x) + ':' + FloatToStr(v.y));
      {$ENDIF}

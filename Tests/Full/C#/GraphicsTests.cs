...
namespace Tests
{
    class GraphicsTests : IGameTestLoader
    {
        ...
        public void AddTo(List<TestSuite> list)
        {
            ...
            result.Add(new TestDrawTriangle());
        ...
        private class TestDrawTriangle : TestSet
        {
            private readonly static string METHS =
                "Triangle Routines";
            ...
            public TestDrawTriangle()
                : base(METHS, INST)
            {
                smallScreen = Graphics.CreateBitmap(418, 418);
                greenTriangle = Shapes.CreateTriangle(0, 200, 50, 200, 50, 150);
                whiteTriangle = Shapes.CreateTriangle(0, 0, 50, 0, 50, 50);
                yellowTriangle = Shapes.CreateTriangle(0, 400, 50, 400, 50, 450);
                triangleRotDeg = 0;
                triangleScale = 1;
                filled = false;
            }
            ...
            Color tempColor;
            float triangleScale;
            float triangleRotDeg;
            Triangle greenTriangle, whiteTriangle, yellowTriangle;

            private void OffsetTempTriangle(Triangle forTriangle)
            {
                tempTriangle = forTriangle;
                m = Physics.Multiply(Physics.TranslationMatrix(tempTriangle[1].X, 0), m);
                Shapes.ApplyMatrix(m, ref tempTriangle);
            }

            private void SetupTempTriangle(Triangle forTriangle)
            {
                tempTriangle = forTriangle;

                midPoint = Shapes.TriangleBarycenter(tempTriangle);
                m = Physics.TranslationMatrix(-midPoint.X, -midPoint.Y);
                m = Physics.Multiply(Physics.ScaleMatrix(triangleScale), m);
                m = Physics.Multiply(Physics.RotationMatrix(triangleRotDeg), m);
                m = Physics.Multiply(Physics.TranslationMatrix(midPoint.X + tempTriangle[1].X, midPoint.Y), m);
            ...
            {
                tempColor = Core.GetColor(0, 255, 0, 120);

                Graphics.ClearSurface(smallScreen, Color.Transparent);

                if (Input.IsKeyPressed(Keys.VK_UP)) triangleScale += 0.1f;
                if (Input.IsKeyPressed(Keys.VK_DOWN)) triangleScale -= 0.1f;
                if (Input.WasKeyTyped(Keys.VK_T)) filled = !filled;
                if (Input.IsKeyPressed(Keys.VK_LEFT)) triangleRotDeg += 5;
                if (Input.IsKeyPressed(Keys.VK_RIGHT)) triangleRotDeg -= 5;

                Graphics.DrawTriangleOn(smallScreen, Color.Green, filled, greenTriangle);

                SetupTempTriangle(greenTriangle);
                Graphics.DrawTriangleOn(smallScreen, tempColor, tempTriangle);

                OffsetTempTriangle(greenTriangle);
                //Graphics.DrawTriangle(smallScreen, tempColor, tempTriangle[0].X, tempTriangle[0].Y, tempTriangle[1].X, tempTriangle[1].Y, tempTriangle[2].X, tempTriangle[2].Y);

                OffsetTempTriangle(greenTriangle);
                Graphics.FillTriangleOn(smallScreen, tempColor, tempTriangle);

                OffsetTempTriangle(greenTriangle);
                //Graphics.FillTriangle(smallScreen, tempColor, tempTriangle[0].X, tempTriangle[0].Y, tempTriangle[1].X, tempTriangle[1].Y, tempTriangle[2].X, tempTriangle[2].Y);

                Graphics.DrawTriangle(Color.White, filled, whiteTriangle);

                tempColor = Core.GetColor(255, 255, 255, 120);

                SetupTempTriangle(whiteTriangle);
                Graphics.DrawTriangle(tempColor, tempTriangle);

                OffsetTempTriangle(whiteTriangle);
                //Graphics.DrawTriangle(tempColor, tempTriangle[0].X, tempTriangle[0].Y, tempTriangle[1].X, tempTriangle[1].Y, tempTriangle[2].X, tempTriangle[2].Y);

                OffsetTempTriangle(whiteTriangle);
                Graphics.FillTriangle(tempColor, tempTriangle);

                OffsetTempTriangle(whiteTriangle);
                //Graphics.FillTriangle(tempColor, tempTriangle[0].X, tempTriangle[0].Y, tempTriangle[1].X, tempTriangle[1].Y, tempTriangle[2].X, tempTriangle[2].Y);
                Graphics.DrawBitmap(smallScreen, 0, 0);

                Graphics.DrawTriangleOnScreen(Color.Yellow, filled, yellowTriangle);
                tempColor = Core.GetColor(255, 255, 0, 120);

                SetupTempTriangle(yellowTriangle);
                Graphics.DrawTriangleOnScreen(tempColor, tempTriangle);

                OffsetTempTriangle(yellowTriangle);
                //Graphics.DrawTriangleOnScreen(tempColor, tempTriangle[0].X, tempTriangle[0].Y, tempTriangle[1].x, tempTriangle[1].y, tempTriangle[2].x, tempTriangle[2].y);

                OffsetTempTriangle(yellowTriangle);
                Graphics.FillTriangleOnScreen(tempColor, tempTriangle);

                OffsetTempTriangle(yellowTriangle);
                //Graphics.FillTriangleOnScreen(tempColor, tempTriangle[0].X, tempTriangle[0].Y, tempTriangle[1].x, tempTriangle[1].y, tempTriangle[2].x, tempTriangle[2].y);
            }
        }
    }
}